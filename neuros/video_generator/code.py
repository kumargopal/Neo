...
# Assuming Neo is always run from the repo root
...
try:
    from lib.video_gen.v_gen import NeoVideoPipeline
except ImportError:
...
async def run(state, *, topic: str = None, duration: int = 60):
    ...
    return {
        "reply": f"🎬 Video generation started! Your video on '{topic}' is being created in the background.\n\n" +
                 f"This may take a few minutes. You can continue using Neo while this runs.\n\n" +
                 f"Task ID: `{task_id}`\n\n" +
